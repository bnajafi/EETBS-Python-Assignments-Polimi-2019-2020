# -*- coding: utf-8 -*-
'''CORREZIONE FATTA: linea 28 aggiunta (devo aggiungere newParsedIndex_weather=newParsedIndex_weather.tz_localize(uguale a se stesso) perchè se no lo modifica e me lo fa vedere ma non permanentemente
                     linea 47 rimossa: non serve shiftare se abbiamo detto alla linea 27 di togliere UTC (greenwich)
                     il problema era che nel file excel non si vedeva ma dal csv si vede un -06 nella data che dava la fascia oraria di austin'''
import pandas as pd
import numpy as np
...
newIndex_weather=newIndex_weather.tz_localize("UTC").tz_convert("America/Mexico_City").tz_localize(None)  
